...
var (
	qlikMetricErrors = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "prometheus_web_qlik_metric_errors_total",
		Help: "Total number of errors that occurred while sending qlik metrics responses.",
	})
	qlikMetricsWarnings = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "prometheus_web_qlik_metric_warnings_total",
		Help: "Total number of warnings that occurred while sending qlik metrics responses.",
	})
)

func registerQlikMetrics(r prometheus.Registerer) {
	r.MustRegister(qlikMetricsWarnings, qlikMetricErrors)
}

func (h *Handler) qlikMetrics(w http.ResponseWriter, req *http.Request) {
	h.mtx.RLock()
	defer h.mtx.RUnlock()
	...
	metrics, _ := parser.ParseMetricSelector("{job!=''}")
	var matcherSets [][]*labels.Matcher
	matcherSets = append(matcherSets, metrics)
	...
	if ws := set.Warnings(); len(ws) > 0 {
		level.Debug(h.logger).Log("msg", "Qlik select returned warnings", "warnings", ws)
		qlikMetricsWarnings.Add(float64(len(ws)))
	}
	if set.Err() != nil {
		qlikMetricErrors.Inc()
	...
	for _, s := range vec {
			...
			if l.Name == labels.MetricName {
				...
				if protMetricFam != nil {
					if err := enc.Encode(protMetricFam); err != nil {
						qlikMetricErrors.Inc()
						level.Error(h.logger).Log("msg", "failed", "err", err)
						return
					}
				}
				protMetricFam = &dto.MetricFamily{
		...
		if !nameSeen {
			level.Warn(h.logger).Log("msg", "Ignoring nameless metric during qlik metric", "metric", s.Metric)
			continue
		}
	...
	if protMetricFam != nil {
		if err := enc.Encode(protMetricFam); err != nil {
			qlikMetricErrors.Inc()
			level.Error(h.logger).Log("msg", "Qlik metrics failed", "err", err)
		}
	}
}
